...
parser = argparse.ArgumentParser(description="Generate an Excel file with word assignments.")
parser.add_argument("num_sheets", type=int, nargs="?", default=1, help="Number of sheets to generate (default is 1).")
...
def assign_words(word_lists, sections, heading_word_list):
    ...
    if heading_word_list in remaining_lists:  # Ensure it's in the list before removing
        remaining_lists.remove(heading_word_list)
...
def populate_worksheet(workbook, worksheet, assignments, heading_words, sarneg_word, sections_with_headings, word_lists):
    # Initialize colors and formatting
    pastel_colors = {
    ...
    auth_sections = {
        ...
        'AuthSection3': ['Challenge', 'Password', 'Running Pass', 'Number Combo']
    }
...
def create_excel(assignments, heading_words, output_filename, sections_with_headings, word_lists, num_sheets=3):
    with pd.ExcelWriter(output_filename, engine='xlsxwriter') as writer:
        ...
        for sheet_index in range(num_sheets):
            ...
            # Generate new assignments for each sheet
            unique_assignments = assign_words(word_lists, sections, heading_words)

            # Select a new heading word list for each sheet
            heading_word_list = random.choice(list(word_lists.keys()))
            heading_words = word_lists[heading_word_list]

            # Select a new Sarneg word for each sheet
            sarneg_word = random.choice(sarneg_words)
            
            # Populate the current worksheet
            populate_worksheet(workbook, worksheet, unique_assignments, heading_words, sarneg_word, sections_with_headings, word_lists)
...
sections = {
    ...
    'Position': ["Forward", "Backward", "Left", "Right", "High", "Low", "North", "South", "East", "West", "Clockwise", "CCW"],
...
# Create Excel file
output_filename = 'assigned_words_multi.xlsx'
create_excel(assignments, heading_words, output_filename, sections_with_headings, word_lists, num_sheets)

print(f'Excel file "{output_filename}" with {num_sheets} sheets created successfully.')
